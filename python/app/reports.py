...
ECHARTS_MIN = (
    (Path(__file__).parent / "assets" / "echarts.min.js").read_text()
    if (Path(__file__).parent / "assets" / "echarts.min.js").exists()
    else None
)
MAX_EMBEDDED_RESOURCE_URL_BYTES = 8 * 1024 * 1024
_INLINE_URL_FETCHER = (
    URLFetcher(allowed_protocols={"data"}, allow_redirects=False, fail_on_errors=True)
    if URLFetcher is not None
    else None
)
...
def _render_section_markdown(md: str) -> str:
    ...
    # Markdown image syntax would otherwise create a second resource-loading
    # channel in PDF rendering, including attacker-controlled data URIs. Report
    # charts are the only images we support and are injected separately below.
...
def _render_chart_divs(chart_specs: list[tuple[str, dict[str, Any]]], static: bool = False) -> str:
    ...
    for cid, spec in chart_specs:
        ...
        encoded_option = html.escape(
            json.dumps(option, ensure_ascii=False, separators=(",", ":")),
            quote=True,
        )
        encoded_id = html.escape(str(cid), quote=True)
        fallback_png = ""
        if ECHARTS_MIN is None:
            png_b64 = charts.render_png_base64(spec)
            fallback_png = f' data-png="data:image/png;base64,{png_b64}"'
        out.append(
            f'<div class="chart-block" id="chart-{encoded_id}" data-option="{encoded_option}"'
...
def build_html(report: dict[str, Any], static: bool = False) -> str:
    ...
    generated_at = html.escape(str(report.get("generated_at") or ""), quote=True)
    sections_html = ""
    ...
    if not ECHARTS_MIN:
        raise RuntimeError("python/app/assets/echarts.min.js is missing; reports require the vendored echarts bundle")
    echarts = ECHARTS_MIN
...
def build_pdf(report: dict[str, Any]) -> bytes:
    ...
    report_html = build_html(report, static=True)
    pdf = WeasyHTML(string=report_html, url_fetcher=_safe_url_fetcher).write_pdf()
    return pdf


def _safe_url_fetcher(url: str):
    """Allow only the trusted inline PNGs generated for static report charts.

    Markdown-origin images are stripped before this point. This deny-by-default
    fetcher is a second boundary that prevents any future report input from
    becoming a filesystem read, metadata-service request, or arbitrary egress.
    """
    if len(url.encode("utf-8")) > MAX_EMBEDDED_RESOURCE_URL_BYTES:
        raise ValueError("embedded report resource exceeds the size limit")
    parsed = urlsplit(url)
    if parsed.scheme.lower() != "data" or not url.lower().startswith("data:image/png;base64,"):
        raise ValueError("external report resources are disabled")
    if _INLINE_URL_FETCHER is None:  # pragma: no cover - guarded by build_pdf
        raise RuntimeError("WeasyPrint unavailable")
    return _INLINE_URL_FETCHER(url)
