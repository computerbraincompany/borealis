# Cohere North research archive (2026-08-22)

> **Historical external research and independent design proposals.** The evidence
> in this directory was captured on 2026-08-22. The 2026-08-28 documentation review
> clarified its archival status and checked local references; it did not recrawl
> the vendor site or verify a newer North release.

**Research date:** 2026-08-22<br>
**Start source:** <https://private.docs.cohere.com><br>
**Original research goal:** provide enough evidence, product behavior, contracts, architecture context, and independent implementation design to rebuild a North-equivalent platform with a fully open-source stack.

## Relationship to Borealis

This archive is not the specification of the implemented Borealis product. Its
enterprise feature set, proposed stack, data model, and roadmap are comparative
research and design options, not an accepted work queue. In particular, the
proposed Kubernetes/PostgreSQL/gateway architecture does not describe the current
Node, SQLite, LanceDB, and Electron implementation.

Use the [Borealis README](../../README.md), [API reference](../API.md),
[desktop guide](../../desktop/README.md), and
[completed implementation plans](../../plans/README.md) for current repository
behavior and its implementation history. Dates, release labels, source counts,
and the word “current” in the archived research refer to the capture date unless
...
This research captured **435 unique public first-party URLs**, fully read **421**, retained **14** explicit soft-not-found aliases, and found **0** authentication-gated pages. The captured raw North OpenAPI 3.1 contract contains **72 paths, 107 operations, and 364 schemas**.[11]

The independent reference architecture proposed a permissive/open stack around React/TypeScript, Fastify, PostgreSQL/pgvector, optional OpenSearch, Valkey, S3-compatible storage, Keycloak, OpenFGA, Temporal, LiteLLM, vLLM/llama.cpp, Docling, gVisor/Kata, MCP, and OpenTelemetry. Component and model licenses require a fresh exact-version review before adopting any proposal.
...
| [02 — Documentation delta at capture](02-current-documentation-delta-and-release-signals.md) | July→August crawl delta and v1.14.0 product/API/ops implications. |
...
| [09 — Public API and object model](09-public-api-and-object-model.md) | Operation and schema inventory generated from the captured North OpenAPI; not the Borealis API. |

### Historical independent rebuild proposals
...
- [Screenshot catalog](screenshots/README.md) — **18** public product UI references verified at capture, plus contact sheet and per-file source/hash metadata.
- [Architecture diagram catalog](architecture-diagrams/README.md) — **11** public GCP/AWS/Azure/OCI reference diagrams verified at capture, plus contact sheet and provenance.
...
- [`research/SOURCE_CATALOG.md`](research/SOURCE_CATALOG.md) — all captured in-scope URLs grouped by area.
...
2. Read **03–09** as the captured North product/technical specification.
3. Read **10–12** as the independent implementation design.
4. Read **13** as a historical proposed build sequence, not an active Borealis implementation plan.
...
The live vendor documentation is mutable. Local documentation maintenance does
not make this evidence current. A future research refresh needs a new capture
date and these checks, with the original snapshot retained for comparison:
