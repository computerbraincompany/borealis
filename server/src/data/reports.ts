...
/**
 * Shared versioned appendix contract (M13). Documents compile an evidence
 * appendix plus a validity-status line through this same normalized report so
 * every renderer (interactive HTML, static HTML, PDF via either backend, and
 * the desktop renderer, which only ever sees already-compiled HTML) stays in
 * sync without widening the 20-section body budget. Legacy payloads omit both
 * fields and normalize exactly as before.
 */
export const REPORT_APPENDIX_MAX_CHARS = 150_000;
export const REPORT_STATUS_MAX_CHARS = 300;
...
export interface NormalizedReport {
  ...
  charts: ReportChart[];
  tables: ReportTable[];
  /** Shared versioned evidence appendix; rendered as a final bounded block. */
  appendix?: string;
  /** Validity-status line shown in the masthead. */
  status?: string;
}
...
export function normalizeReport(reportValue: unknown, requireAccount = false): NormalizedReport {
  ...
  const appendixValue = reportValue.appendix;
  const appendix = appendixValue === undefined ? undefined : boundedString(appendixValue, REPORT_APPENDIX_MAX_CHARS);
  const statusValue = reportValue.status;
  const status = statusValue === undefined ? undefined : boundedString(statusValue, REPORT_STATUS_MAX_CHARS);
...
export function buildHtml(reportValue: unknown, options: BuildHtmlOptions = {}): string {
  ...
  const statusPill = report.status ? `<span class="pill">${escapeHtml(report.status)}</span>` : "";
  const appendix = report.appendix
    ? `<div class="section"><div class="appendix">${renderSectionMarkdown(report.appendix)}</div></div>`
    : "";
