...
After fresh unsigned packaging, run
`pnpm test:e2e:product:desktop --native-driver=external` for the
...
An OS UI driver must complete the [native checkpoints](../scripts/e2e/NATIVE_DESKTOP.md);
omitting the driver reports the requested native journeys as blocked.
...
diagnostics plus parsed host policy and exact main/helper signature checks;
setup errors, timeouts, output overflow and cleanup failures cannot
