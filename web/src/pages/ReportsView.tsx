      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Self-contained HTML and PDF reports generated by North from your chats.
          </p>
        </div>
      ...
      {loading ? (
        ...
        <Card className="mt-8 flex flex-col items-center gap-3 py-16 text-center">
          ...
          <p className="text-sm text-muted-foreground">
            No reports yet. Ask North in a chat to "create a report" and it will land here as HTML + PDF.
          </p>
        </Card>
      ) : (
        <div className="mt-8 space-y-3">
          {reports.map((r) => (
            <Card key={r.id} className="flex items-center gap-4 p-4 transition-colors hover:border-white/10">
      ...
      <Dialog open={!!preview} onOpenChange={(o) => !o && setPreview(null)}>
        <DialogContent className="flex max-h-[85vh] max-w-4xl flex-col overflow-hidden p-0">
          <DialogHeader className="px-6 pt-5">
            ...
            <DialogDescription>Standalone HTML export — renders identically to the PDF.</DialogDescription>
          </DialogHeader>
          <div className="min-h-0 flex-1 overflow-hidden px-6 pb-6">
            {previewHtml ? (
              ...
              <div className="flex h-[calc(70vh-2rem)] items-center justify-center rounded-xl border text-sm text-muted-foreground">
                {previewErr}
              </div>
            ) : (
