...
export function ReportsView() {
  ...
  const load = useCallback(async () => {
    ...
    setChartsError(null);
    try {
      // Each catalog fails independently: reports stay usable when only the
      // chart registry errors, and one load pass must not double-fetch reports.
      const [reportPage, chartRows] = await Promise.allSettled([reportsApi.list(), chartsApi.list()]);
      if (mountedRef.current && reportsRequestId === reportsCatalogRequestRef.current) {
        if (reportPage.status === "fulfilled") {
          setReports((current) => mergeCatalogHead(reportPage.value.items, current));
          reportsNextCursorRef.current = reportPage.value.next_cursor;
          setReportsNextCursor(reportPage.value.next_cursor);
        } else {
          setPageError(formatApiError(reportPage.reason, "Could not load reports"));
        }
        if (chartRows.status === "fulfilled") {
          setCharts(chartRows.value);
        } else {
          setChartsError(formatApiError(chartRows.reason, "Could not load the chart gallery"));
        }
      }
    } finally {
              ...
              return (
                <Card key={r.id} className="flex items-center gap-4 p-4 transition-colors hover:border-foreground/20">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {r.subtitle && <div className="mt-1 truncate text-xs text-muted-foreground">{r.subtitle}</div>}
                ...
                <Card key={shared.id} className="flex items-center gap-4 p-4">
                  ...
                  <div className="flex shrink-0 items-center gap-1.5">
                    ...
                    <Button
                      ...
                      size="icon"
                      title="Download PDF"
                      aria-label={`Download PDF of ${shared.title}`}
                      className="text-muted-foreground hover:text-primary"
                      onClick={() =>
          ...
          <section className="mt-12" aria-labelledby="chart-gallery-heading">
            ...
            {charts.length > GALLERY_LIMIT && (
              <p className="mt-3 text-xs text-muted-foreground">
                Showing the newest {GALLERY_LIMIT} of {charts.length} charts. Older charts stay available in their
                source chats.
              </p>
            )}
          </section>
