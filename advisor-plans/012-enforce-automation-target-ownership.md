...
- The planned-at schema version was 11 (`server/src/db/migrations.ts:3`).
  Completed plan 031 owns v12, and dependency plan 006 is expected to own v13.
  This plan must add exactly v14 and must not reuse or renumber either predecessor.

- Dependency plan 003 establishes one migration framework: `server/src/tests/sqliteMigrationFixture.ts`, immutable deltas `server/src/tests/fixtures/sqlite/v001.sql` through `v012.sql`, and assertions in `server/src/tests/sqliteFoundation.test.ts`. Dependency plan 006 must add contiguous `v013.sql`. This plan adds `v014.sql`; do not create another fixture harness.
...
| ------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------- |
| Dependency check                | `test -f server/src/tests/sqliteMigrationFixture.ts && test -f server/src/tests/fixtures/sqlite/v013.sql && node -e "const f=require('node:fs').readdirSync('server/src/tests/fixtures/sqlite').filter(x=>/^v\\d{3}\\.sql$/.test(x));if(f.length<13)process.exit(1)"` | exit 0                                                              |
...
- `server/src/tests/fixtures/sqlite/v014.sql` (create; exact immutable copy of the v14 delta)
...
- `server/src/tests/sqliteMigrationFixture.ts` and v001-v013 fixture files; they are dependency-owned and immutable.
...
- Any schema version other than v14. Plan 031 owns v12, Plan 006 owns v13, and the later reserved sequence must remain contiguous.
...
Confirm plan 003's helper/fixture contract exists, plan 006 has added `v013.sql`, and `LATEST_SQLITE_SCHEMA_VERSION` is exactly 13 before this work. Confirm migrations 1-13 and fixtures v001-v013 are contiguous with no extra v14. Read the dependency plan's migration behavior before editing.

**Verify**: run the Dependency check command, then `rg -n 'LATEST_SQLITE_SCHEMA_VERSION|version: 13|SCHEMA_V13' server/src/db/migrations.ts` → the latest version is 13 and exactly one v13 migration is registered after plan 031's v12.

### Step 2: Add schema v14 with kind-specific owned targets

Add `SCHEMA_V14` and advance `LATEST_SQLITE_SCHEMA_VERSION` from 13 to 14. Rebuild both `automations` and `automation_runs` in one migration; SQLite cannot add these composite foreign keys in place.
...
- v13 can contain multiple otherwise-valid `connector_sync` rows for one owned
  connector (the store calls these legacy multiples). Before creating the v14
...
- copy runs only for automations copied into v14;
...
unordered `GROUP BY` behavior. Create temporary v14 tables, copy only the valid
...
the exact same SQL delta in `server/src/tests/fixtures/sqlite/v014.sql`.

**Verify**: `pnpm --filter borealis-server exec vitest run --config vitest.integration.config.ts src/tests/sqliteFoundation.test.ts` → fixture inventory reaches v14, historical upgrades pass, and `PRAGMA foreign_key_check` is empty.

### Step 3: Write migration and database-constraint regressions

Using only plan 003's fixture helper and `sqliteFoundation.test.ts`, add a v13→v14 upgrade case seeded before migration with:
...
Remove the best-effort `deleteConnectorAutomations(...).catch(...)` block from `server/src/routes/connectors.ts`. Connector deletion should reserve/delete sources and the connector in its existing transaction; the v14 foreign key then removes the automation and its `automation_runs`. Connector sync history continues to cascade through its existing v10 composite foreign key.

Update `connectorRoutes.test.ts` raw setup to use v14 target columns and replace the legacy-multiple/cleanup assumptions that the unique constraint now makes impossible. Make the cascade test prove automation and run removal even when no manual helper is called.
...
Run the typecheck, lint, format, full tests, and final repository gate. Inspect all SQL in both the migration constant and v014 fixture byte-for-byte for semantic equality.

Audit current-code inserts with
`rg -n -U 'INSERT\s+INTO\s+automations\s*\([^)]*\btarget_id\b' server/src --glob '*.ts'`.
Manually classify every match: only immutable pre-v14 migration SQL and an
explicit v13 historical-fixture seed may write legacy `target_id`; current
...
**Verify**: `git diff --check && git status --short` → no whitespace errors; only the seven in-scope paths (including new `v014.sql`) plus the permitted `advisor-plans/README.md` status update are modified.

## Test plan

- Historical fixture upgrade from v13 containing valid and invalid legacy
...
- The explicit multiline SQL audit verifies that only historical v13 setup can
  still insert legacy `target_id`; full server and repository gates then cover
  the current schema behavior.

## Done criteria

- [ ] Schema latest is exactly 14 after plan 031's v12 and dependency plan 006's v13.
- [ ] `v014.sql` exactly matches `SCHEMA_V14`; v001-v013 remain untouched.
...
- [ ] Valid v13 automations/history migrate; deterministic legacy connector
...
- Plans 003, 006, and 031 are not DONE, `LATEST_SQLITE_SCHEMA_VERSION` is not exactly 13, `v013.sql` is missing, or any other plan has already claimed v14.
...
- The v13 duplicate-survivor rule cannot be expressed deterministically in the
...
- Keep the `created_at DESC, id DESC` duplicate-survivor fixture permanently. Removing
  ...
  failure.
- Schema numbering is shared across plans. Future migrations must start at v15; never edit v14 after release.
