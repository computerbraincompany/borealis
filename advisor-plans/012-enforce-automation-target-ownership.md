...
> Plans 003, 006, and 031 are expected to change some of these paths. Confirm their stated postconditions below, then compare all live automation schema/store excerpts before proceeding. Unexpected schema numbering is a STOP condition.
> Plans 024, 025, and 026 also intentionally changed connector cleanup
> authority, automation terminal history, and protected route parsing. Preserve
> the exact-location cleanup queue, cancel-as-skipped semantics, `onRequest`
> authentication, and route-owned body limits. These are expected baseline
> changes, not permission to STOP or restore predecessor behavior.
...
- **Effort**: M
- **Risk**: MED
- **Depends on**: `advisor-plans/003-add-historical-migration-fixtures.md`, `advisor-plans/006-bind-egress-consent-to-provider-revision.md`, `advisor-plans/031-paginate-resource-catalogs.md`
- **Preserve completed baseline**: Plans 024, 025, 026, and 031
- **Category**: migration
- **Planned at**: commit `f1b9293`, 2026-08-30

## Why this matters

Automation target ownership is currently enforced only by application prechecks. SQLite can retain an automation after its connector or chat is deleted, causing scheduled failures, ambiguous connector schedules, and manual best-effort teardown that can silently fail. A v15 schema rebuild with kind-specific composite foreign keys makes the ledger authoritative, gives connector/chat deletion reliable cascades, and preserves the public `target_id` contract.
...
- The planned-at schema version was 11 (`server/src/db/migrations.ts:3`).
  Completed plan 031 owns v12, and dependency plan 006 is expected to own v14.
  This plan must add exactly v15 and must not reuse or renumber either predecessor.

- Dependency plan 003 establishes one migration framework: `server/src/tests/sqliteMigrationFixture.ts`, immutable deltas `server/src/tests/fixtures/sqlite/v001.sql` through `v012.sql`, and assertions in `server/src/tests/sqliteFoundation.test.ts`. Dependency plan 006 must add contiguous `v014.sql`. This plan adds `v015.sql`; do not create another fixture harness.
...
| ------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------- |
| Dependency check                | `test -f server/src/tests/sqliteMigrationFixture.ts && test -f server/src/tests/fixtures/sqlite/v014.sql && node -e "const f=require('node:fs').readdirSync('server/src/tests/fixtures/sqlite').filter(x=>/^v\\d{3}\\.sql$/.test(x));if(f.length<13)process.exit(1)"` | exit 0                                                              |
...
- `server/src/tests/fixtures/sqlite/v015.sql` (create; exact immutable copy of the v15 delta)
...
- `server/src/tests/sqliteMigrationFixture.ts` and v001-v014 fixture files; they are dependency-owned and immutable.
...
- Any schema version other than v15. Plan 031 owns v12, Plan 006 owns v14, and the later reserved sequence must remain contiguous.
...
Confirm plan 003's helper/fixture contract exists, plan 006 has added `v014.sql`, and `LATEST_SQLITE_SCHEMA_VERSION` is exactly 14 before this work. Confirm migrations 1-14 and fixtures v001-v014 are contiguous with no extra v15. Read the dependency plan's migration behavior before editing.

**Verify**: run the Dependency check command, then `rg -n 'LATEST_SQLITE_SCHEMA_VERSION|version: 14|SCHEMA_V14' server/src/db/migrations.ts` → the latest version is 14 and exactly one v14 migration is registered after the agent editor's v13.

### Step 2: Add schema v15 with kind-specific owned targets

Add `SCHEMA_V15` and advance `LATEST_SQLITE_SCHEMA_VERSION` from 14 to 15. Rebuild both `automations` and `automation_runs` in one migration; SQLite cannot add these composite foreign keys in place.
...
- v14 can contain multiple otherwise-valid `connector_sync` rows for one owned
  connector (the store calls these legacy multiples). Before creating the v15
...
- copy runs only for automations copied into v15;
...
unordered `GROUP BY` behavior. Create temporary v15 tables, copy only the valid
...
the exact same SQL delta in `server/src/tests/fixtures/sqlite/v015.sql`.

**Verify**: `pnpm --filter borealis-server exec vitest run --config vitest.integration.config.ts src/tests/sqliteFoundation.test.ts` → fixture inventory reaches v15, historical upgrades pass, and `PRAGMA foreign_key_check` is empty.

### Step 3: Write migration and database-constraint regressions

Using only plan 003's fixture helper and `sqliteFoundation.test.ts`, add a v14→v15 upgrade case seeded before migration with:
...
Remove the best-effort `deleteConnectorAutomations(...).catch(...)` block from `server/src/routes/connectors.ts`. Connector deletion should reserve/delete sources and the connector in its existing transaction; the v15 foreign key then removes the automation and its `automation_runs`. Connector sync history continues to cascade through its existing v10 composite foreign key.

Update `connectorRoutes.test.ts` raw setup to use v15 target columns and replace the legacy-multiple/cleanup assumptions that the unique constraint now makes impossible. Make the cascade test prove automation and run removal even when no manual helper is called.
...
Run the typecheck, lint, format, full tests, and final repository gate. Inspect all SQL in both the migration constant and v015 fixture byte-for-byte for semantic equality.

Audit current-code inserts with
`rg -n -U 'INSERT\s+INTO\s+automations\s*\([^)]*\btarget_id\b' server/src --glob '*.ts'`.
Manually classify every match: only immutable pre-v15 migration SQL and an
explicit v14 historical-fixture seed may write legacy `target_id`; current
...
**Verify**: `git diff --check && git status --short` → no whitespace errors; only the seven in-scope paths (including new `v015.sql`) plus the permitted `advisor-plans/README.md` status update are modified.

## Test plan

- Historical fixture upgrade from v14 containing valid and invalid legacy
...
- The explicit multiline SQL audit verifies that only historical v14 setup can
  still insert legacy `target_id`; full server and repository gates then cover
  the current schema behavior.

## Done criteria

- [ ] Schema latest is exactly 15 after plan 031's v12 and dependency plan 006's v14.
- [ ] `v015.sql` exactly matches `SCHEMA_V15`; v001-v014 remain untouched.
...
- [ ] Valid v14 automations/history migrate; deterministic legacy connector
...
- Plans 003, 006, and 031 are not DONE, `LATEST_SQLITE_SCHEMA_VERSION` is not exactly 14, `v014.sql` is missing, or any other plan has already claimed v15.
...
- The v14 duplicate-survivor rule cannot be expressed deterministically in the
...
- Keep the `created_at DESC, id DESC` duplicate-survivor fixture permanently. Removing
  ...
  failure.
- Schema numbering is shared across plans. Future migrations must start at v16; never edit v15 after release.
