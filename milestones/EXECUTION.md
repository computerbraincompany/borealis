...
| ---- | ----- | --------------------- | --------------------- |
| Prerequisite closure 001,003–009,011–016,020 | IN PROGRESS | 001 `50d70b7`, 003 `795ebad`, 011 `042bec1`, 004 `9c14f40`, 009 `f3dc381`, 015 `5640f66`, 005 `c150ac7`, 006 `418f7a9` (v14), 012 `642032d` (v15), 013 `1a6be8e`, 007 `394ba6b` (+ web 007b `41a10e3` adaptation), 008 `fb99667` | 12 of 13 merged 2026-09-06 after per-diff review; merged-main gates green after each batch (967 unit + 97 integration post-008; web 320 + budgets; policy/desktop-verify pass at baseline). 012/013/007/008 passed full root `pnpm verify` (or full suites+desktop) in worktrees before merge; 007's single serverApp.test.ts hoist-mock conflict resolved by the coordinator during rebase (both sides kept). 014 (owned application runtime, incl. desktop verify) executing; then 016 → 020(v16). Phase B v17+ pre-allocations will be recorded here at merge time (plan: v17 connections/MCP infra, v18 M12 — actual integration order rules). Protocol: one subagent per plan in `../north-clone-wt/<plan>`, plan-scoped ownership; coordinator reviews diffs, rebases in-worktree + ff-merges, runs integrated gates. Load flake: re-run 5 s-budget subprocess tests before treating as failure. |
