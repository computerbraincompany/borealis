...
| ---- | ----- | --------------------- | --------------------- |
| Prerequisite closure 001,003–009,011–016,020 | IN PROGRESS | 001 `50d70b7`, 003 `795ebad`, 011 `042bec1`, 004 `9c14f40`, 009 `f3dc381`, 015 `5640f66`+`2c3e0c1`, 005 `c150ac7` | 7 of 13 merged to main 2026-09-06 after per-diff review; integrated gates green after each batch (final merged-main run: 863 unit + 92 integration, typecheck/lint/format pass; web 316 + build budgets + policy pass; desktop `verify` pass at baseline). Plan 006 (real schema v14) executing in worktree from `c150ac7`. Remaining order: 006 → 007/012(v15)/013 → 008 → 014 → 016 → 020(v16). Protocol: one subagent per plan in `../north-clone-wt/<plan>` worktree with plan-scoped file ownership; coordinating agent reviews diffs, merges serially (rebase-in-worktree + ff-merge), runs integrated gates. Worktree-load flake: subprocess-heavy tests (e.g. `workspaceServerLock`) can hit 5 s budgets under ≥3 concurrent suites; re-run before treating as failure. |
